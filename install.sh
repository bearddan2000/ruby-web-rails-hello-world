...
function start-up(){
    ...
    pid=`sudo docker ps | grep $docker_img_name | cut -d ' ' -f 1`
    info_base="[$timestamp INFO]: $basefile::$scope"
    ...
    cp src/* bin \
        && cd bin \
        && cat main.tar.gz-part-* > main.tar.gz \
        && tar -xf main.tar.gz \
        && chmod -R +x * \
        && cd ../ \
        && sleep 3
    ...
    sudo docker run -d --rm --name test -p 80:3000 $docker_img_name

    sudo docker exec test rake db:migrate
